...
def register_user(request):
    ...
    if request.method == 'POST':
        form = CustomUsuarioCreationForm(request.POST, request.FILES) # Para manejar la foto
        if form.is_valid():
            user = form.save(commit=False)
            # Puedes asignar roles por defecto aquí si no se marcan en el formulario
            # user.es_jugador = True # Por ejemplo, si todos los registros son jugadores
            user.save()
    ...
    return render(request, 'users/register.html', {'form': form}) # Asegúrate de tener este template
