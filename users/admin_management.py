...
def admin_management(request):
    ...
    if search_query:
        ...
        ).filter(es_admin_aso=False, is_superuser=False)[:10]  # Limit to 10 results
    else:
        available_users = []
...
def promote_to_admin(request, user_id):
    ...
    if request.method == "POST":
        ...
        if user.es_admin_aso or user.is_superuser:
            ...
            user.save()
            messages.success(
                request, f"{user.get_full_name} ha sido promovido a administrador."
            )
...
def demote_from_admin(request, user_id):
    ...
    if request.method == "POST":
        ...
        if user.is_superuser:
            ...
            user.is_staff = False
            user.save()
            messages.success(request, f"{user.get_full_name} ya no es administrador.")
